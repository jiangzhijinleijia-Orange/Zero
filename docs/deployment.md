...
### 1.1 お金の流れ — 誰に、どこから請求されるのか

**大原則:どのサービスも「カード(支払い方法)を登録した相手」にしか請求できない。** カードを登録していなければ、請求は発生しようがなく、サービスは超過時に"止まる"だけ。現在、どのサービスにもカードは登録していない=請求が落ちる先がそもそも存在しない。

- 請求関係は **「運用者 ↔ 各サービス」の直接の二者関係**で、しかも**カードを入れた相手とだけ**成立する。Supabase は Supabase に、Vercel は Vercel に、それぞれ別々にカードを登録して初めて課金され得る。一方を有料化しても、他方には影響しない。
- **開発(Claude Code)は支払いの通り道ではない。** コードを書いて公開するだけで、運用者の財布とインフラ費用は一切繋がっていない。「開発を仲介してインフラ費用が請求される」という構造は存在しない。
- 運用者が払う唯一の固定費は、開発に Claude を使うための Anthropic 利用料のみ。これは**アプリの利用量と無関係**で、生徒が何人使っても増えず、Supabase/Vercel の費用がここを通ることもない(完全に別系統)。

**将来、意図的に発生させ得るコスト**(いずれも「自分でカードを登録し、有料プランを選ぶ」という能動的な操作が前提。勝手には発生しない):

| 何に | どこへ支払う | 金額の目安 |
|---|---|---|
| Supabase Pro(容量・機能の拡張、日次バックアップ等) | Supabase | $25/月〜 |
| Vercel Pro(商用利用・各種上限の拡張) | Vercel | $20/月〜 |
| 独自ドメイン(例: `ryukyu30.jp` のような名前) | **ドメイン業者**(お名前.com 等。Vercel ではない) | 年 1,000〜3,000円 |
| Phase 2 の AI 学習提案(LLM を使う場合) | 利用する AI 事業者 | 従量(使った分だけ) |

**Discord**(OAuth・Webhook)と、**公開リポジトリの GitHub Actions** は、そもそも課金の仕組みがなく、請求され得ない。
